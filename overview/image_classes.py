...
class CustomImageFolder(Dataset):
    ...
    def __init__(self, img_dir, csv_dir, transform=TF):
        ...
        self.data = [
            ...
            if bool(re.search(r"\.(jp[e]?g|png)$", file))
            and self.csv[self.csv["ObjectID"] == int(file.split(".")[0])][
                "Classification"
            ].size
            == 1
        ]  # if the image has a class in the csv file.
    ...
    def __getitem__(self, idx):
        try:
            ...
            label = np.array(
                self.csv[self.csv["ObjectID"] == img_id]["Classification"]
            )[0]
            return img, label
